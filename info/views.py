...
class SubscriptionDetail(APIView):
    @swagger_auto_schema(operation_description='subject 검색시 참조 : apt(기본값), remndr, urbty', query_serializer=detail_serializer)
...
class SubscriptionNearby(APIView):
    @swagger_auto_schema(query_serializer=nearby_serializer)
    def get(self, request):
        # 현재위치 기반 찾기
        ...
        if api_test.status_code == 200:
            ...
            return JsonResponse({
                'value': 'location is unkown param',
                'timestamp': datetime.today()
            })
